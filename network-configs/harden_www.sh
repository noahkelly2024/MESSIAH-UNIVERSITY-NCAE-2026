...
if command -v apache2 &>/dev/null || dpkg -l apache2 &>/dev/null 2>&1; then
    ...
    a2dismod mpm_prefork mpm_worker 2>/dev/null || true   # Disable old MPM modules
    a2enmod mpm_event 2>/dev/null || a2enmod mpm_prefork  # Enable MPM module (required for Apache to start)
